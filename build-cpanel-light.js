// build-cpanel-light.js
const { execSync } = require('child_process');
const fs = require('fs');
const path = require('path');

const rootDir = __dirname;
const deployDir = path.join(rootDir, 'cpanel_deploy_light');
const zipFile = path.join(rootDir, 'deploy_cpanel_light.zip');

console.log('=== Starting LIGHT cPanel Build & Package Process ===');
...
// Step 2: Prepare cpanel_deploy_light directory
console.log('\nStep 2: Preparing clean cpanel_deploy_light folder...');
if (fs.existsSync(deployDir)) {
  console.log('Removing old cpanel_deploy_light directory...');
  fs.rmSync(deployDir, { recursive: true, force: true });
}
fs.mkdirSync(deployDir, { recursive: true });

// Step 3: Copy necessary files and folders (EXCLUDING node_modules)
console.log('\nStep 3: Copying files to deployment directory (excluding node_modules)...');
const pathsToCopy = [
  '.next',
  'public',
  'prisma',
...
// Step 4.5: Clean up build cache and unnecessary folders to reduce size
...
// Step 5: Compress into deploy_cpanel_light.zip
console.log('\nStep 5: Zipping deploy folder to deploy_cpanel_light.zip...');
if (fs.existsSync(zipFile)) {
  console.log('Removing old deploy_cpanel_light.zip...');
  fs.unlinkSync(zipFile);
}

try {
  console.log('Using tar to compress...');
